...
void LedsPlayActionServer::execute(
    ...
    auto feedback = std::make_shared<hri_interfaces::action::LedsPlay::Feedback>();
    auto result = std::make_shared<hri_interfaces::action::LedsPlay::Result>();


    if (mode == hri_interfaces::msg::LedModes::STEADY) {
        steadyMode(goal_handle, leds, frequency, colors, intensities);

    } else if (mode == hri_interfaces::msg::LedModes::BLINKING) {
    ...
    } else if (mode == hri_interfaces::msg::LedModes::LOOP) {
